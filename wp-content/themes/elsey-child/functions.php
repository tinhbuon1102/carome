...
function elsey_change_cssjs_ver( $src ) {
	if( strpos( $src, '?ver=' ) )
		...
		$src = add_query_arg( array('ver' => '2.0'), $src );
		return $src;
}
...
function remove_product_editor() {
	...
	if (isset($_GET['test_epsilon']))
	{
		$epsilon_response = get_post_meta($_GET['test_epsilon'], 'epsilon_response_array', true);
		$epsilon_data = array();
		...
		// $response_array = $epsilon_request->get_order_to_epsilon( $order ,$epsilon_data['contract_code'], $epsilon_data['st_code'], true, $gateway_id,
		// $epsilon_data['conveni_code'] );
