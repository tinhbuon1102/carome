    ...
    function elsey_product_categories_function( $atts, $content = NULL ) {
	    ...
	    ob_start();
	      ...
	      foreach ($pro_cats as $pro_cat) {
	        ...
	        $output_masonry .= '<div class="els-cat-masonry-text els-catsc-text"><div class="els-cat-info-box test">';
	    ...
	    <?php
	      ...
	      foreach ($pro_cats as $pro_cat) {
	        ...
	        $output_default .= '<div class="els-cat-default-box '. esc_attr($column_class) .'">';
	        ...
	        $output_default .= '<div class="els-catdt-box"><a href="'.esc_url($catUrl).'"></a>';
	        $output_default .= ($cat_image) ? '<div class="els-catdt-img"><img src="'.esc_url($cat_image).'" alt="'.esc_attr($pro_cat->name).'"></div>' : '<div class="els-catdt-img"></div>'; 
	        $output_default .= '<div class="masonry-grid__item__copy"><div class="els-cat-info-box testchild">';

	        if( $pro_cat->name ) {
	          $output_default .= '<h3 class="masonry-grid__item__title"><a href="'.esc_url($catUrl).'">'.esc_attr($pro_cat->name);
	          $output_default .= ($cat_pr_count) ? ' ['.esc_attr($pro_cat->count).']' : '';
	          $output_default .= '</a></h3>';
			  $output_default .= '<a href="'.esc_url($catUrl).'">Shop '.esc_attr($pro_cat->name).'';
	        }
	        ...
	        $output_default .= '</div></div></div></div>';
