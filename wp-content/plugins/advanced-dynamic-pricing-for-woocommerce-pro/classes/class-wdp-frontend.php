...
class WDP_Frontend {
	...
	public function woocommerce_product_is_on_sale( $on_sale, $product ) {
		...
		if ( $product->is_type( 'variable' ) ) {
			return apply_filters( 'wdp_product_variation_is_on_sale', $on_sale, $this->woocommerce_variable_price_html( '', $product ), $product );
		} else {
			...
			$last_product_cached_result = $this->woocommerce_get_price_html( '', $product );
		}
		if ( $last_product_cached_result ) {
	...
	public function woocommerce_variable_price_html( $price_html, $product, $qty = 1 ) {
		...
		foreach ( $product->get_children() as $child_id ) {
			...
			$prices        = self::get_initial_and_discounted_price( $child, $qty );
	...
	public function woocommerce_get_price_html( $price_html, $product, $qty = 1 ) {
		...
		if ( is_a( $product, 'WC_Product' ) ) {
			...
			$prices        = self::get_initial_and_discounted_price( $product, $qty );
			if ( ! $prices ) {
				return $price_html;
			}
	...
	public function woocommerce_product_get_sale_price($value, $product) {
		...
		//calculate 
		$prices        = self::get_initial_and_discounted_price( $product, 1 );
...
// 		$cart = new WDP_Cart( array(), $coupons, $context );
	 ...
	 */
	public static function get_initial_and_discounted_price( $product, $qty = 1 ) {
		$calc = self::make_wdp_calc_from_wc();
		$cart = self::make_wdp_cart_from_wc();
	 ...
	 * @return WDP_Cart
	 */
	private static function make_wdp_cart_from_wc($use_empty_cart = false) {
		$context = self::make_wdp_cart_context_from_wc();
		if( $use_empty_cart ) 
			$cart = new WDP_Cart( array(), array(), $context );
		else
	...
	public static function get_active_rules_for_product( $product_id, $qty = 1, $use_empty_cart = false ) {
		$calc = self::make_wdp_calc_from_wc();
		$cart = self::make_wdp_cart_from_wc( $use_empty_cart );
