...
class WDP_Cart {
	...
	/**
	 * Calculated original quantities grouped by product_id
	 * @var array $items_qty_by_product_id
	 */
	private $items_qty_by_product_id = array();

	/**
	 * Cache for bulk which qty based on categories
	 * @var array $calculated_original_items_qty_with_categories
	 */
	private $calculated_original_items_qty_with_categories = array();
	...
	public function __construct( $items, $external_coupons, $context ) {
		...
		$this->calculate_qty_original_cart_item_by_product_id();
	...
	public function get_item_prices_to_display( $uid ) {
		...
		// no rules has been applied, skip
		if ( isset( $item['rules'] ) && ! sizeof( $item['rules'] ) ) {
			return false;
		}
	...
	private function group_processed_items() {
		...
		foreach ( $this->processed_items as $cart_item ) {
			...
			// fix serialize float with large precision creates different key for same rules
			$rules                    = md5( serialize( array_map( function ( $item ) {
				return round( $item, 5 );
			}, $cart_item['rules'] ) ) );
			$original_rules[ $rules ] = $cart_item['rules'];

			$variation = isset( $cart_item['variation'] ) ? serialize( $cart_item['variation'] ) : serialize(array());

			// store wdp_rules and wdp_rules_for_singular to prevent split items that affected by the same bulk rule
			...
			if ( isset( $cart_item['original_item']['wdp_rules_for_singular'] ) ) {
				$wdp_rules_for_singular = $cart_item['original_item']['wdp_rules_for_singular'];
				unset( $cart_item['original_item']['wdp_rules_for_singular'] );
			} else {
				$wdp_rules_for_singular = false;
			}
					...
					foreach ( $grouped_by_rules as $rules => $grouped_by_price ) {
						foreach ( $grouped_by_price as $price => $grouped_by_original_item ) {
							foreach ( $grouped_by_original_item as $original_item => $qty ) {
								...
								$temp_rules = $original_rules[$rules];
								...
								if ( isset( $wdp_rules_for_singular ) && $wdp_rules_for_singular !== false ) {
									$temp_original_item['wdp_rules_for_singular'] = $wdp_rules_for_singular;
								}
								$temp_rules_for_singular = $temp_rules;
								...
								$this->grouped_processed_items[] = array(
									...
									'gifted'             => $is_gifted,
									'rules'              => $temp_rules,
									'rules_for_singular' => $temp_rules_for_singular,
...
//				'data'     		=> $product,
			...
			$cart_item_key = WDP_Frontend::process_without_hooks( function () use ( $wc_cart, $product_id, $cart_item, $variation_id, $variation, $cart_item_data ) {
				return $wc_cart->add_to_cart( $product_id, $cart_item['quantity'], $variation_id, $variation, $cart_item_data );
			}, $exclude_hooks );
...
//		}
	...
	public function get_original_price( $product_id, $item_meta = false ) {
		...
		if ( isset( $this->cached_original_price[ $product_id ] ) && ! has_filter( 'wdp_get_product_price' ) ) {
	...
	public function calculate_qty_original_cart_item_by_product_id() {
		if ( ! empty( $this->items_qty_by_product_id ) ) {
			return $this->items_qty_by_product_id;
		}


		foreach ( $this->original_items as $item ) {
			$product_id = $item['product_id'];
			$qty        = $item['quantity'];
			if ( ! isset( $this->items_qty_by_product_id[ $product_id ] ) ) {
				$this->items_qty_by_product_id[ $product_id ] = 0;
			}
			$this->items_qty_by_product_id[ $product_id ] += $qty;
		}

		return $this->items_qty_by_product_id;
	}

	public function get_original_items_qty_with_categories( $categories_ids ) {
		if ( empty( $categories_ids ) ) {
			return 0;
		}

		sort( $categories_ids, SORT_NUMERIC );
		$hash = md5( json_encode( $categories_ids ) );

		if ( isset( $this->calculated_original_items_qty_with_categories[ $hash ] ) ) {
			return $this->calculated_original_items_qty_with_categories[ $hash ];
		}

		$qty = 0;
		foreach ( $this->items_qty_by_product_id as $product_id => $product_qty ) {
			$parent_product_id = wp_get_post_parent_id( $product_id );
			$product_id        = $parent_product_id ? $parent_product_id : $product_id;
			...
			if ( array_intersect( $categories_ids, wp_list_pluck( $terms, 'term_id' ) ) ) {
				$qty += $product_qty;
			}
		}

		$this->calculated_original_items_qty_with_categories[ $hash ] = $qty;
	...
	public static function make_from_wc_cart( $wc_cart, $context ) {
		...
		foreach ( $wc_cart_items as $wc_cart_item_key => $wc_cart_item ) {
			...
			$item = apply_filters( 'wdp_prepare_cart_item', array(
				'product_id'     => $product_id,
				'quantity'       => apply_filters( 'wdp_get_product_qty', $wc_cart_item['quantity'], $wc_cart_item ),
			...
			if ( apply_filters( 'wdp_skip_cart_item', false, $item, $product) ) {
				continue;
			}
