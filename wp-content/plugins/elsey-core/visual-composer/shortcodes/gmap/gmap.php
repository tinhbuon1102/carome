...
/* ==========================================================
  Gmap
=========================================================== */
if ( !function_exists('elsey_gmap_function')) {
  function elsey_gmap_function( $atts, $content = NULL ) {
    ...
    if ($gmap_common_marker) {
      ...
      $common_marker = ELSEY_PLUGIN_ASTS . '/images/map-marker.png';
    }
    ...
    // Google Map Height
    if ( $gmap_height ) {
      $inline_style .= '.elsey-gmap-'. $e_uniqid .' {';
      $inline_style .= ( $gmap_height ) ? 'height:'. elsey_check_px($gmap_height) .';' : '';
      $inline_style .= '}';
    }

    // add inline style
    add_inline_style( $inline_style );
    $styled_class  = ' elsey-gmap-'. $e_uniqid;

    wp_enqueue_script( 'elsey-map-api', '//maps.googleapis.com/maps/api/js'. $gmap_api .'', array( 'jquery' ), '', false );
    wp_enqueue_script( 'elsey-google-map', ELSEY_PLUGIN_ASTS . '/jquery.googlemap.js', array( 'jquery' ), '1.5.0', false );
    ...
    // Map Tab ID
    if ($gmap_id) {
      $gmap_open = '<div '. $gmap_id .' class="elsey-gmap-tab tab-pane fade">';
      $gmap_close = '</div>';
    } else {
      $gmap_open = '';
      $gmap_close = '';
    ...
    echo $gmap_open;
    echo '<div id="map-'. $e_uniqid .'" class="elsey-google-map '. $custom_css . $class . $styled_class .'"></div>';
    echo $gmap_close;
...
add_shortcode( 'elsey_gmap', 'elsey_gmap_function' );
