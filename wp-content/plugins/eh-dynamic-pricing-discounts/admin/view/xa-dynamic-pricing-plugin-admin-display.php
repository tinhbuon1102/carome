...
if(isset($_REQUEST['submit']))
{
    ...
    $path = xa_dy_root_path . 'admin/data/settings_page/save-options.php';
...
if (isset($_REQUEST['update']) && empty($_REQUEST['update'])) {    //Submit And Not Edit Then Saving New Record
    ...
    $path = xa_dy_root_path . 'admin/data/' . $current_tab_loc . 'save-options.php';
    if (file_exists($path) == true)
        include_once ( $path );
}
elseif (isset($_REQUEST['edit']) && !empty($_REQUEST['edit'])) {    //Loading Edit Form Or Updating Data
    ...
    $current_tab_loc = isset($_REQUEST['tab']) && !empty($_REQUEST['tab']) ? $_REQUEST['tab'] . '/' : 'product_rules/';
    $path = xa_dy_root_path . 'admin/data/' . $current_tab_loc . 'load-edit.php';
    include_once ( $path );
    ...
    $path = xa_dy_root_path . 'admin/data/' . $path . 'update-options.php';
    include_once ( $path );
} else {
    $active_tab = 'product_rules';
}
