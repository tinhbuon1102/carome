...
?>
    <div style="float: right;padding: 10px;">
        ...
        ?>
        Page: 
        <select style="display: inline-block;" onchange="page_dropdown_func('buy_get_free_rules');" id="page_dropdown">
            <?php
            for($i=1;$i<=$pages;$i++)
            {
                $selected= '';
        ...
        <button type="button" <?php echo $prevdisable;?> class='<?php echo $classprev;?>' onclick="new_page(<?php echo $current_page_no-1;?>,'buy_get_free_rules');"></button>
        <button type="button" <?php echo $nextdisable;?> class='<?php echo $classnext;?>' onclick="new_page(<?php echo $current_page_no+1;?>,'buy_get_free_rules');"></button>
    </div>
    <table class="display_all_rules table widefat" style=" border-collapse: collapse;">
        <thead style="font-size: smaller;background-color: lightgrey;">
            ...
            <th class="xa-table-header" style=""><?php esc_attr_e('Purchased Product', 'eh-dynamic-pricing-discounts'); ?></th>
