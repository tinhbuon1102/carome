...
class RP_WCDPD_Controller_Methods_Cart_Discount extends RP_WCDPD_Controller_Methods
{
    ...
    public function discount_is_valid_for_cart($is_valid, $coupon)
    {
        ...
        // Discount is not valid for whole cart if discount is calculated per item
        return !in_array($this->applicable_adjustments[$coupon_code]['rule']['pricing_method'], array('discount_per_cart_item__amount', 'discount_per_cart_item__percentage'), true);
    }
    ...
    public function get_discount_amount($discount, $price_to_discount, $cart_item, $single, $coupon)
    {
        ...
        if (isset($rule) && $rule['pricing_method'] === 'discount_per_cart_item__amount') {
            $conditions = !empty($rule['conditions']) ? $rule['conditions'] : array();
            $subtotal_excl_tax = RP_WCDPD_Conditions::get_sum_of_cart_item_subtotals_by_product_conditions($conditions, false);
        }
        else {
