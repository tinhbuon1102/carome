...
function wad_get_customer_facebook_shared_links() {
    $links = array();
    if (isset($_SESSION["social_data"]["facebook"]["posts"]["data"]))
        $links = array_map(create_function('$o', 'return $o["link"];'), $_SESSION["social_data"]["facebook"]["posts"]["data"]);
    return $links;
}
...
/**
    ...
    function wad_get_hybrid_config(){
        if(!class_exists("Hybrid_Auth"))
            ...
            require_once( WAD_DIR . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'hybridauth' . DIRECTORY_SEPARATOR . 'Hybrid' . DIRECTORY_SEPARATOR . 'thirdparty' . DIRECTORY_SEPARATOR . 'Facebook' . DIRECTORY_SEPARATOR . 'Facebook.php' );
        return WAD_DIR . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'hybridauth' . DIRECTORY_SEPARATOR . 'config.php';
    }

    function wad_is_user_likes_facebook_page($page_id){
        $is_liked = false;
               ...
               $config = wad_get_hybrid_config();
                try {
                    ...
                    $adapter = $hybridauth->authenticate("facebook");
                        ...
                        $api=$adapter->api();
                        $encoded_data=$api->get("/me/likes/$page_id", $token);
                        $decode_data = $encoded_data->getDecodedBody();
                        if(!empty($decode_data["data"])){
                            $_SESSION["social_data"]["facebook"]["likes"] = $decode_data;
                            $is_liked = true;
                        }
                } catch (Exception $e) {
