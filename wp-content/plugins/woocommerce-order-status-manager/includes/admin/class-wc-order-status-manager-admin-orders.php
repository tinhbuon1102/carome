...
class WC_Order_Status_Manager_Admin_Orders {
	...
	public function __construct() {
		...
		if ( SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.2.0' ) ) {
	...
	public function custom_email_order_actions( $actions ) {
		...
		if ( ! empty( $custom_emails ) ) {
			...
			$actions = SV_WC_Helper::array_insert_after( $actions, 'send_order_details', $new_actions );
		}
	...
	public function send_custom_order_email( $order ) {
		...
		if ( ! empty( $_POST['wc_order_action'] ) ) {
			...
			if ( SV_WC_Helper::str_starts_with( $action, 'send_osm_email_' ) ) {
				...
				WC()->payment_gateways();
				WC()->shipping();
				WC()->mailer()->emails["wc_order_status_email_{$email_post->ID}"]->trigger( $order->get_id(), $order );
