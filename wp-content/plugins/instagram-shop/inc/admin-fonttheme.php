<?php
class Instashop_Admin_Fonttheme extends Instashop_Admin_Base {
	...
	public function fonttheme_setting() {
		...
		if ( false !== $auth_param['instashop_auth']['auth_status'] ) {
	...
	public function get_font_select_form() {
		$options = get_option( 'instashop_custom_theme' );
		$edit_theme = false;
		if ( isset( $_POST['edit_theme'] ) ) {
			$edit_theme = esc_attr( $_POST['edit_theme'] );
		}
		$fonts = Instashop_Fonts::get_instance();
		/* @TODO Debug Code
			$option_name = 'instashop_fonttheme';
		...
		if ( $theme_id ) {
			$theme_input = "<input type='hidden' name='instashop_custom_theme[id]' value='{$theme_id}' />";
			...
			$html .= "<input type='hidden' name='instashop_custom_theme[id]' value='{$theme_id}' maxlength='16' style='width:100%;' pattern='^[0-9A-Za-z]+$' required/>";
		}
		$html .= '<h4>■'. __( 'フォントテーマ名' , self::$text_domain ). '</h4>';
		$html .= '<p>'. __( 'フォントテーマの名称を設定します。（最大16文字まで）' , self::$text_domain ). '</p>';
		$html .= "<input type='text' name='instashop_custom_theme[name]' value='{$theme_name}' maxlength='16' style='width:100%;' required/>";
...
jQuery( document ).ready(function() {
	jQuery( form_id ).submit(function() {
		var title = jQuery( 'select[name="instashop_custom_theme[fonts][title][font]"]' ).val();
		var lead = jQuery( 'select[name="instashop_custom_theme[fonts][lead][font]"]' ).val();
		var text = jQuery( 'select[name="instashop_custom_theme[fonts][text][font]"]' ).val();
		var bold = jQuery( 'select[name="instashop_custom_theme[fonts][bold][font]"]' ).val();
		if (
			title === 'false' &&
			lead === 'false' &&
			text === 'false' &&
...
EOM;
		$html = '<script>';
		$endpoint = path_join( IS_PLUGIN_URL , 'inc/font.json' );
