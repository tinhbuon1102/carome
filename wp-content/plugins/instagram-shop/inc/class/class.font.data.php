...
class Instashop_Fonts {
	...
	public function load_all_font_data() {
		$font_theme = Instashop_Fonttheme::get_instance();
	...
	public function get_selected_fonttheme() {
		...
		return $fonththeme['instashop_themes'];
	}

	public function get_selected_post_fonttheme( $post_id ) {
		$meta = get_post_meta( $post_id , 'instashop_fonttheme' , true );
		if ( isset( $meta['theme'] ) ) {
			$theme = $meta['theme'];
	...
	public function get_fonttheme_params() {
		$param['instashop_themes'] = $this->get_fonttheme_options();
		$param['instashop_themes_keys'] = $this->get_fonttheme_keys();
		return $param;
	}

	public function update_font_setting() {
		if ( ! isset( $_POST['instashop_custom_theme'])) {
			return;
		}

		$options = get_option( 'instashop_custom_theme' );
		$options = $this->parse_font_setting_param( $options, $_POST['instashop_custom_theme'] );

		if(isset($options['theme']['']['id'])){
			$this->update_font_theme_setting();
		}else{
			update_option( 'instashop_custom_theme', $options );
		}
	...
	public function delete_custom_theme() {
		...
		$options = get_option( 'instashop_custom_theme' );
		$id = $_POST['instashop_custom_theme']['id'];
	...
	public function update_font_theme_setting() {
		if ( ! isset( $_POST['instashop_fonttheme'] ) || isset( $_POST['ts_change_edit_theme'] )) {
			return;
		}
		$options = get_option( 'instashop_fonttheme' );
		foreach ( $_POST['instashop_fonttheme'] as $key => $target ) {
			$key = esc_attr( $key );
			$options[ $key ] = esc_attr( $target );
		}
		if ( isset( $_POST['instashop_fonttheme']['fade_time'] ) ) {
			if ( ! isset( $_POST['instashop_fonttheme']['fade_in'] ) ) {
				$options['fade_in'] = false;
			}
		}
		if($options['font_theme'] === 'new'){
			$options['font_theme'] = $_POST['instashop_custom_theme']['id'];
		}
		update_option( 'instashop_fonttheme', $options );
	}
