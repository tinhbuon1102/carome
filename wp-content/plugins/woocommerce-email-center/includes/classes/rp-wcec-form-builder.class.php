 ...
 * Form Builder Class
 *
 * @class RP_WCEC_Form_Builder
 * @package WooCommerce Email Center
 * @author RightPress
 */
if (!class_exists('RP_WCEC_Form_Builder')) {
...
class RP_WCEC_Form_Builder
{
    ...
    {
        // Get attributes
        $attributes = self::attributes($params, array('value', 'maxlength', 'placeholder', 'rows'), 'textarea', $context);
    ...
    public static function date($params, $context = 'admin')
    {
        ...
        self::input('text', $params, array('value', 'placeholder'), $context, true);
    }
    ...
    private static function label_html($params)
    {
        ...
        if (!empty($params['id']) && !empty($params['label'])) {
            ...
            // Field is required
            // $required_html = !empty($params['required']) ? ' <abbr class="required" title="' . __('required', 'rp_wcec') . '">*</abbr>' : '';
            $required_html = '';
    ...
    {
        $date_class = $is_date ? 'rp_wcec_date_container' : '';
