...
<table class="wdp_pricing_table">
	...
	<?php foreach ( $data['ranges'] as $line ): ?>
        <tr>
            <td>
	            <?php echo apply_filters( 'wdp_format_bulk_record', ($line['from'] == $line['to'] ) ? $line['from'] : ( empty($line['to']) ? $line['from'] . ' +': $line['from'] . ' - ' . $line['to'] ) , $line); ?>
            </td>
	        <?php if ( $show_product_discount_in_bulk_table ): ?>
                <td>
	                <?php
                ...
                <ul>
                    <?php foreach ( $dependencies as $dependency ): ?>
                        <li>
                            ...
                            } elseif ( in_array( $dependency['type'], array_keys( WDP_Helpers::get_custom_product_taxonomies() ) ) ) {
	                            if ( 'in_list' === $dependency['method'] ) {
		                            echo sprintf(
			                            __( '%d product(s) with ' . $dependency['type'] . ' from list: %s',
				                            'advanced-dynamic-pricing-for-woocommerce' ),
			                            $dependency['qty'],
			                            $items
		                            );
		                            ...
		                            echo sprintf(
			                            __( '%d product(s) with ' . $dependency['type'] . ' not from list: %s',
				                            'advanced-dynamic-pricing-for-woocommerce' ),
			                            $dependency['qty'],
			                            $items
		                            );
