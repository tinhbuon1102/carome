	...
	class Pie_WCWL_Waitlist_Archive {
		...
		public function return_html_for_archive_list( $product_id, $archives ) {
			...
			foreach ( $archives as $date => $waitlist ) {
				$html .= '<li class="wcwl_archive_' . $product_id . '" ><h4>' . __( 'The following users were mailed an in-stock notification on ', 'woocommerce-waitlist' ) . date( "F j, Y", $date ) . '<div class="dashicons dashicons-email-alt wcwl_email_all_tab"></div>
				...
				foreach ( $waitlist as $user_id ) {
					...
					if ( $userdata ) {
						$html .= '<tr><td class="wcwl_archive_email">' . $userdata->user_email . $this->get_user_language_flag( $user_id, $product_id ) . '</td>';
						...
						$html .= '<tr><td class="wcwl_archive_email">' . __( 'User doesn\'t exist', 'woocommerce-waitlist' ) . '</td>';
