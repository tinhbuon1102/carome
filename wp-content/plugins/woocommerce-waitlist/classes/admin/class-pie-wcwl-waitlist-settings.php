	...
	class Pie_WCWL_Waitlist_Settings {
		...
		public function enqueue_settings_scripts( $hook ) {
			if ( 'woocommerce_page_wc-settings' == $hook && isset( $_GET['section'] ) && 'waitlist' == $_GET['section'] ) {
				wp_enqueue_script( 'wcwl_admin_settings', Pie_WCWL_Compatibility::plugin_url() . '/includes/js/wcwl_admin_settings.js' );
				$data = array(
					...
					'update_desc'             => __( 'Update Waitlist Counts', 'woocommerce-waitlist' ),
					'update_button_text'      => __( 'Update Now', 'woocommerce-waitlist' ),
					'update_warning'          => sprintf( __( '%sWarning:%s This could take a long while depending on how many products there are. Navigating away from this page will stop the update.', 'woocommerce-waitlist' ), '<b>', '</b>' ),
					'update_message'          => sprintf( __( 'Currently updating waitlist counts for product %s', 'woocommerce-waitlist' ), '<span class="wcwl_current_update"></span>/<span class="wcwl_total_updates"></span>' ),
					'update_message_complete' => sprintf( __( 'Successfully updated waitlist counts for %s products', 'woocommerce-waitlist' ), '<span class="wcwl_total_updates"></span>' ),
				);
				wp_localize_script( 'wcwl_admin_settings', 'wcwl_settings', $data );
				wp_enqueue_style( 'wcwl_admin_settings', Pie_WCWL_Compatibility::plugin_url() . '/includes/css/wcwl_admin_settings.css' );
			}
		}
