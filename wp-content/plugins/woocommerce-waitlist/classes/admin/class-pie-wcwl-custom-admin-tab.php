...
if ( ! class_exists( 'Pie_WCWL_Custom_Tab' ) ) {
	...
	class Pie_WCWL_Custom_Tab {
		...
		public function enqueue_scripts() {
			wp_enqueue_style( 'wcwl_admin_custom_tab_css',Pie_WCWL_Compatibility::plugin_url() . '/includes/css/wcwl_admin_custom_tab.css' );
			wp_enqueue_script( 'wcwl_admin_custom_tab_js', Pie_WCWL_Compatibility::plugin_url() . '/includes/js/wcwl_admin_custom_tab.js', array(), '1.0.0', true );
			wp_localize_script( 'wcwl_admin_custom_tab_js', 'wcwl_tab', array( 'invalid_email' => __( 'Email address is invalid', 'woocommerce-waitlist' ) ) );
		}
		...
		public function custom_tab_options_tab() {
			...
			<li class="wcwl_waitlist_tab">
				<a href="#wcwl_waitlist_data"><span><?php _e( 'Waitlists', 'woocommerce-waitlist' ); ?></span></a>
			</li>
			<?php
		}
		...
		public function build_custom_tab() {
			...
			if ( ! empty( $users ) || ! $this->product->is_in_stock() ) {
				...
				if ( empty( $users ) ) {
					...
					echo '<th>' . __( 'User', 'woocommerce-waitlist' ) . '</th>';
					echo '<th>' . __( 'Added', 'woocommerce-waitlist' ) . '</th>';
					echo '<th>' . __( 'Email', 'woocommerce-waitlist' ) . '</th>';
					echo '<th>' . __( 'Remove', 'woocommerce-waitlist' ) . '</th></tr>';
					...
					$emails = $this->waitlist->get_registered_users();
				...
				if ( $variation ) {
					...
					if ( ! empty( $users ) || ! $variation->is_in_stock() ) {
						...
						if ( empty( $users ) ) {
							...
							$emails = $variation_waitlist->get_registered_users();
							if (is_array($emails) && count($emails))
							{
		...
		public function return_user_info( $user, $date, $waitlist ) {
			...
			$language = $this->get_user_language_flag( $user->ID, $waitlist->product_id );
			return '<tr>
					<td><strong><a title="' . esc_attr( $this->view_user_profile_text ) . '" href="' . admin_url( 'user-edit.php?user_id=' . $user->ID ) . '">' . $user->display_name . '</a>' . $language . '</strong></td>
					<td>' . $date . '</td>
		 ...
		 * @return string
		 */
		private function get_user_language_flag( $user_id, $product_id ) {
		...
		public function add_users_to_waitlist( $waitlist ) {
			...
			foreach ( $emails as $email ) {
				...
				$current_user = get_user_by( 'id', WooCommerce_Waitlist_Plugin::create_new_customer_from_email( $email ) );
		...
		private function get_mailto_link_content( $waitlist ) {
			$emails = $waitlist->get_registered_users();
			if (is_array($emails) && count($emails))
			{
