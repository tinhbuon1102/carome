...
if ($product->is_type('variable')):
    ...
    ?>
    <div class="<?php echo esc_attr(implode(' ', array_map('sanitize_html_class', $wrapper_classes))); ?> images row thumbs-position-<?php echo esc_attr($thums_position); ?> image-action-<?php echo esc_attr($image_action); ?>"
        ...
        <div class="<?php if ($attachment_ids && $thums_position == 'left' && !$is_quick_view): ?>col-md-9 col-md-push-3<?php else: ?>col-sm-12<?php endif ?>">
            ...
            <figure class="woocommerce-product-gallery__wrapper <?php if (woodmart_is_main_product_images_carousel()) echo woodmart_owl_items_per_slide(1) . ' owl-carousel'; ?>">
                ...
                if (has_post_thumbnail($variation_id)) {
                    $html = '<div class="product-image-wrap"><figure data-thumb="' . get_the_post_thumbnail_url($variation_id, 'shop_thumbnail') . '" class="woocommerce-product-gallery__image"><a href="' . esc_url($full_size_image[0]) . '">';
                    $html .= get_the_post_thumbnail($variation_id, $thumbnail_size, $attributes);
                    $html .= '</a></figure></div>';
                } else {
