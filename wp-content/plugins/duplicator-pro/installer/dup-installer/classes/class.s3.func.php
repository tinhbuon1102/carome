...
final class DUPX_S3_Funcs
{
    ...
    private static function getS3dataFilePath()
    {
        static $path = null;
        if (is_null($path)) {
            $path = $GLOBALS['DUPX_INIT'].'/dup-installer-s3data__'.DUPX_Package::getPackageHash().'.json';
        }
        return $path;
    }
    ...
    private function setGlobalSearchAndReplaceList()
    {
        ...
        if ($GLOBALS['DUPX_AC']->defineValueExists('WP_HOME')) {
            $old_urls_list[] = $GLOBALS['DUPX_AC']->getDefineValue('WP_HOME');
        }
        if ($GLOBALS['DUPX_AC']->defineValueExists('WP_SITEURL')) {
            $old_urls_list[] = $GLOBALS['DUPX_AC']->getDefineValue('WP_SITEURL');
        }
        if ($GLOBALS['DUPX_AC']->realValueExists('siteUrl')) {
            $old_urls_list[] = $GLOBALS['DUPX_AC']->getRealValue('siteUrl');
        }
        if ($GLOBALS['DUPX_AC']->realValueExists('homeUrl')) {
            $old_urls_list[] = $GLOBALS['DUPX_AC']->getRealValue('homeUrl');
        }
    ...
    public function configurationFileUpdate()
    {
        ...
        try {
            if (file_exists(DUPX_Package::getWpconfigArkPath())) {
                ...
                if ($confTransformer->exists('constant', 'WP_DEBUG') || $this->post['wp_debug']) {
                ...
                if ($confTransformer->exists('constant', 'WP_DEBUG_LOG') || $this->post['wp_debug_log']) {
                ...
                // WP_DEBUG_DISPLAY
                if ($confTransformer->exists('constant', 'WP_DEBUG_DISPLAY') || $this->post['wp_debug_display']) {
                ...
                // SCRIPT_DEBUG
                if ($confTransformer->exists('constant', 'SCRIPT_DEBUG') || $this->post['script_debug']) {
                ...
                if ($confTransformer->exists('constant', 'SAVEQUERIES') || $this->post['savequeries']) {
                ...
                // WP_AUTO_UPDATE_CORE
                if (!empty($this->post['wp_auto_update_core'])) {
