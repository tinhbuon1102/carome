...
class DUPX_InstallerState
{
    ...
    {
        if (($data = DupProSnapJsonU::wp_json_encode($this)) === false) {
            DUPX_Log::info('Installer state json encode error');
            throw new Exception('Installer state json encode errore');
        }
        $result = DupProSnapLibIOU::filePutContents(self::$state_filepath, $data);
        if ($result) {
            DUPX_Log::info('INSTALLER STATE SAVE OK', DUPX_Log::LV_DETAILED);
        } else {
            DUPX_Log::info('INSTALLER STATE SAVE ERROR', DUPX_Log::LV_DEFAULT);
        }
        return $result;
    }
}