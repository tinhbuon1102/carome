...
class DUPX_Boot
{
    ...
    public static function init()
    {
        ...
        self::initArchive();
        DUPX_InstallerState::init(self::isInit());
        if (!($GLOBALS['DUPX_STATE'] = DUPX_InstallerState::getInstance(false))) {
            throw new Exception("Can't initialize installer state");
        }
    ...
    public static function includes()
    {
        ...
        require_once($GLOBALS['DUPX_INIT'].'/classes/utilities/class.ignorant.recursive.iterator.php');
        ...
        require_once($GLOBALS['DUPX_INIT'].'/classes/utilities/class.u.orig.files.manager.php');
        require_once($GLOBALS['DUPX_INIT'].'/classes/utilities/params/class.u.params.manager.php');
        require_once($GLOBALS['DUPX_INIT'].'/classes/config/class.security.php');
        require_once($GLOBALS['DUPX_INIT'].'/classes/plugins/class.plugins.manager.php');
        ...
        require_once($GLOBALS['DUPX_INIT'].'/classes/config/class.conf.utils.php');
        require_once($GLOBALS['DUPX_INIT'].'/classes/class.installer.state.php');
        require_once($GLOBALS['DUPX_INIT'].'/classes/class.logging.php');
        require_once($GLOBALS['DUPX_INIT'].'/ctrls/classes/class.ctrl.ajax.php');
        require_once($GLOBALS['DUPX_INIT'].'/ctrls/classes/class.ctrl.params.php');
        require_once($GLOBALS['DUPX_INIT'].'/ctrls/ctrl.base.php');
        require_once($GLOBALS['DUPX_INIT'].'/ctrls/ctrl.s1.extraction.php');
        require_once($GLOBALS['DUPX_INIT'].'/views/classes/class.view.php');
    ...
    protected static function initInstallerFiles()
    {
        if (!chdir($GLOBALS['DUPX_INIT'])) {
            ...
            throw new Exception("Can't change to directory ".$GLOBALS['DUPX_INIT']);
        }
        ...
        if (self::isInit()) {
            ...
            if (!($GLOBALS['DUPX_STATE'] = DUPX_InstallerState::getInstance(true))) {
                throw new Exception("Can't initialize installer state");
            }

            $paramManager = DUPX_Paramas_Manager::getInstance();
            $paramManager->resetParams();
            $paramManager->initParamsOverwrite();
            self::initParamsBase();
            ...
            $origFManager = DUPX_Orig_File_Manager::getInstance();
            $origFManager->init(false);
            $origFManager->restoreAll();
            $origFManager->save();
        } else {
            self::initParamsBase();
    ...
    {
        // don't use param manager because isn't inizialized
        return isset($_REQUEST[DUPX_Paramas_Manager::PARAM_CTRL_ACTION]) && $_REQUEST[DUPX_Paramas_Manager::PARAM_CTRL_ACTION] === "ctrl-step0";
    }
