...
class DUP_PRO_Archive_File_List
{
    ...
    public function __construct($path)
    {
        ...
        $this->path = DUP_PRO_u::safePath($path);
    }
    ...
    public function addEntry($path, $size, $nodes)
    {
        ...
        fwrite($this->handle, DupProSnapJsonU::wp_json_encode($entry)."\n");
    }
    ...
    public function getEntriesFormPath($path, $cleanCache = false)
    {
        if (!$this->loadCache()) {
            return false;
        }
    ...
    public function getArrayPaths($pathPrefix = '')
    {
        ...
        $safePrefix = DUP_PRO_U::safePathUntrailingslashit($pathPrefix);
