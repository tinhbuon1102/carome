        ...
        <li id="woocs_li_<?php echo $hash ?>_<?php echo $curr_code ?>">
            <div class="woocs_price_col">
                <p class="form-field form-row restriction_min__field">
                    <label for="woocs_restriction_min_<?php echo $hash ?>_<?php echo $curr_code ?>"><?php _e('Minimum spend', 'woocommerce-currency-switcher') ?>&nbsp;(<b><?php echo $curr_code ?></b>):</label>
                    <input type="text" class="short wc_input_price" name="woocs_restriction_min[<?php echo $post_id ?>][<?php echo $curr_code ?>]" id="woocs_restriction_min_<?php echo woocs_short_id($post_id) ?>_<?php echo $curr_code ?>" value="<?php echo($amouth_min > 0 ? $amouth_min : '') ?>" placeholder="<?php _e('auto', 'woocommerce-currency-switcher') ?>">
                </p>
                <p class="form-field form-row _restriction_max_field">
                    <label for="woocs_restriction_max_<?php echo $hash ?>_<?php echo $curr_code ?>"><?php _e('Maximum spend', 'woocommerce-currency-switcher') ?>&nbsp;(<b><?php echo $curr_code ?></b>):</label>
                    <input type="text" class="short wc_input_price" name="woocs_restriction_max[<?php echo $post_id ?>][<?php echo $curr_code ?>]" id="woocs_restriction_max_<?php echo woocs_short_id($post_id) ?>_<?php echo $curr_code ?>" value="<?php echo($amouth_max > 0 ? $amouth_max : '') ?>" placeholder="<?php _e('auto', 'woocommerce-currency-switcher') ?>">
                </p>
            </div>
            <div class="woocs_price_col">
                <p class="form-row">
                    <a href="javascript:woocs_remove_li_fixed_field('<?php echo $hash ?>','<?php echo $curr_code ?>',false,'<?php echo $type ?>');void(0);" class="button"><?php _e('Remove', 'woocommerce-currency-switcher') ?></a>
                </p>
            </div>
        </li>
        <?php
...
<div class="woocs_multiple_simple_panel options_group pricing woocommerce_variation" >
    ...
    <?php if ($is_fixed_enabled): ?>
        <div id="woocs_tab_fixed_restriction_<?php echo woocs_short_id($post_id) ?>" class="woocs_tab">
            ...
            &nbsp;<a href="javascript:woocs_add_fixed_field('<?php echo $post_id ?>','<?php echo $type ?>', '<?php echo woocs_short_id($post_id) ?>');void(0);" class="button"><?php _e('Add', 'woocommerce-currency-switcher') ?></a>
            &nbsp;<a href="javascript:woocs_add_all_fixed_field('<?php echo $post_id ?>','<?php echo $type ?>', '<?php echo woocs_short_id($post_id) ?>');void(0);" class="button"><?php _e('Add all', 'woocommerce-currency-switcher') ?></a>
            <br />
            <br />
