...
function woocs_add_fixed_field(post_id,selector) {
    var code = jQuery('#woocs_multiple_simple_select_'+selector +'_'+ post_id).val();
    if (code) {
        var html = jQuery('#woocs_multiple_simple_tpl_'+selector).html();
        html = html.replace(/__POST_ID__/gi, post_id);
        html = html.replace(/__CURR_CODE__/gi, code);
        jQuery('#woocs_multiple_simple_list_'+selector +'_' + post_id).append(html);
        //***
        jQuery("#woocs_multiple_simple_select_"+selector +'_' + post_id + " option[value='" + code + "']").remove();
        jQuery("#woocs_multiple_simple_select_"+selector +'_' + post_id + " option").eq(0).prop('selected', 'selected');
    }
    woocs_enable_save_variation_changes();
}

function woocs_add_all_fixed_field(post_id,selector) {
    jQuery.each(jQuery("#woocs_multiple_simple_select_"+selector +"_" + post_id + " option"), function (i, code) {
        jQuery("#woocs_multiple_simple_select_"+selector +"_" + post_id + " option[value='" + code + "']").prop('selected', 'selected');
        woocs_add_fixed_field(post_id,selector);
    });
}
...
function woocs_add_select_fixed_field(post_id, code,selector) {
    jQuery('#woocs_multiple_simple_select_'+selector +'_' + post_id).append('<option value="' + code + '">' + code + '</option>');
}
...
function woocs_remove_li_fixed_field(post_id, code, geo,selector) {

    if (geo) {
        //code is time here
        jQuery('#woocs_multiple_simple_list_geo_'+selector +'_' + post_id + ' #woocs_li_geo_' + post_id + '_' + code).remove();
    } else {
        jQuery('#woocs_multiple_simple_list_'+selector +'_' + post_id + ' #woocs_li_' + post_id + '_' + code).remove();
    }

    woocs_add_select_fixed_field(post_id, code,selector);
    woocs_enable_save_variation_changes();
}
