...
if ( ! class_exists( 'AM_License_Menu' ) ) {
	class AM_License_Menu {
		 ...
		 */
		public static function instance( $file, $software_display_title, $software_title, $software_version, $plugin_or_theme, $api_url, $text_domain = '', $extra = '' ) {
			if ( is_null( self::$_instance ) ) {
				self::$_instance = new self( $file, $software_display_title, $software_title, $software_version, $plugin_or_theme, $api_url, $text_domain, $extra );
			}

			return self::$_instance;
		}

		public function __construct( $file, $software_display_title, $software_title, $software_version, $plugin_or_theme, $api_url, $text_domain, $extra ) {
			$this->file            = $file;
			$this->software_title  = $software_title;
			$this->software_display_title  = $software_display_title;
			...
			if ( is_admin() ) {
				...
				$this->ame_settings_menu_title         = $this->software_display_title . __( ' Activation', $this->text_domain );
				$this->ame_settings_title              = $this->software_display_title . __( ' API Key Activation', $this->text_domain );
			...
			if ( isset($this->ame_activated_key) and get_option( $this->ame_activated_key ) != 'Activated' ) {
		...
		public function inactive_notice() { ?>
			...
			<div id="message" class="error">
				<p><?php printf( __( 'The <strong>%s</strong> API Key has not been activated, so the %s is inactive! %sClick here%s to activate <strong>%s</strong>.', $this->text_domain ), esc_attr( $this->software_display_title ), esc_attr( $this->plugin_or_theme ), '<a href="' . esc_url( admin_url( 'options-general.php?page=' . $this->ame_activation_tab_key ) ) . '">', '</a>', esc_attr( $this->software_display_title ) ); ?></p>
			</div>
			<?php
		}
		...
		public function check_external_blocking() {
			...
			if ( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
				...
				if ( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
					?>
					<div class="error">
						<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', $this->text_domain ), $this->software_display_title, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>' ); ?></p>
					</div>
					<?php
				}
			}
			...
			if ( $_REQUEST[ 'option_page' ] != $this->ame_deactivate_checkbox ) {
				if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $api_email == '' || $checkbox_status == 'on' || $current_api_key != $api_key ) {
					...
					if ( $activate_results[ 'activated' ] === true && ! empty( $this->ame_activated_key ) ) {
						add_settings_error( 'activate_text', 'activate_msg', sprintf( __( '%s activated. ', $this->text_domain ), esc_attr( $this->software_display_title ) ) . "{$activate_results['message']}.", 'updated' );
		...
		public function expired_license_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The license key for %s has expired. You can reactivate or purchase a license key from your account <a href="%s" target="_blank">dashboard</a>.', $this->text_domain ) . '</p></div>', esc_attr( $this->software_display_title ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function on_hold_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s is on-hold. You can reactivate the subscription from your account <a href="%s" target="_blank">dashboard</a>.', $this->text_domain ) . '</p></div>', esc_attr( $this->software_display_title ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function cancelled_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s has been cancelled. You can renew the subscription from your account <a href="%s" target="_blank">dashboard</a>. A new license key will be emailed to you after your order has been completed.', $this->text_domain ) . '</p></div>', esc_attr( $this->software_display_title ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function expired_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s has expired. You can reactivate the subscription from your account <a href="%s" target="_blank">dashboard</a>.', $this->text_domain ) . '</p></div>', esc_attr( $this->software_display_title ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function suspended_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s has been suspended. You can reactivate the subscription from your account <a href="%s" target="_blank">dashboard</a>.', $this->text_domain ) . '</p></div>', esc_attr( $this->software_display_title ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function pending_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s is still pending. You can check on the status of the subscription from your account <a href="%s" target="_blank">dashboard</a>.', $this->text_domain ) . '</p></div>', esc_attr( $this->software_display_title ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function trash_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s has been placed in the trash and will be deleted soon. You can purchase a new subscription from your account <a href="%s" target="_blank">dashboard</a>.', $this->text_domain ) . '</p></div>', esc_attr( $this->software_display_title ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function no_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'A subscription for %s could not be found. You can purchase a subscription from your account <a href="%s" target="_blank">dashboard</a>.', $this->text_domain ) . '</p></div>', esc_attr( $this->software_display_title ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function no_key_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'A license key for %s could not be found. Maybe you forgot to enter a license key when setting up %s, or the key was deactivated in your account. You can reactivate or purchase a license key from your account <a href="%s" target="_blank">dashboard</a>.', $this->text_domain ) . '</p></div>', esc_attr( $this->software_display_title ), esc_attr( $this->software_display_title ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function download_revoked_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'Download permission for %s has been revoked possibly due to a license key or subscription expiring. You can reactivate or purchase a license key from your account <a href="%s" target="_blank">dashboard</a>.', $this->text_domain ) . '</p></div>', esc_attr( $this->software_display_title ), esc_url( $this->ame_renew_license_url ) );
		}
		...
		public function no_activation_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( '%s has not been activated. Go to the settings page and enter the license key and license email to activate %s.', $this->text_domain ) . '</p></div>', esc_attr( $this->software_display_title ), esc_attr( $this->software_display_title ) );
		}
		...
		public function switched_subscription_error_notice() {
			echo sprintf( '<div id="message" class="error"><p>' . __( 'You changed the subscription for %s, so you will need to enter your new API License Key in the settings page. The License Key should have arrived in your email inbox, if not you can get it by logging into your account <a href="%s" target="_blank">dashboard</a>.', $this->text_domain ) . '</p></div>', esc_attr( $this->software_display_title ), esc_url( $this->ame_renew_license_url ) );
		}
