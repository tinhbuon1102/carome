 ...
 *
 * Provides a Epsilon GMO Postpay Payment Payment Gateway (Link Type).
 *
 * @class 			WC_Epsilon
 * @extends		WC_Gateway_Epsilon_Pro_GP
 * @version		1.0.0
 * @package		WooCommerce/Classes/Payment
 * @author			Artisan Workshop
 */
class WC_Gateway_Epsilon_Pro_GP extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->id                = 'epsilon_pro_gp';
		$this->has_fields        = false;
		$this->method_title      = __( 'Epsilon GMO Postpay Payment', 'wc4jp-epsilon' );
		...
		$this->st_code = '00000-0000-00000-00010-00000-00000-00000';

        // Create plugin fields and settings
		$this->init_form_fields();
		$this->init_settings();
		$this->method_title       = __( 'Epsilon GMO Postpay Payment', 'wc4jp-epsilon' );
		$this->method_description = __( 'Allows payments by Epsilon GMO Postpay Payment in Japan.', 'wc4jp-epsilon' );

		// Get setting values
		foreach ( $this->settings as $key => $val ) $this->$key = $val;

		// Actions
		add_action( 'woocommerce_receipt_epsilon_pro_gp',                              array( $this, 'receipt_page' ) );
       ...
       */
	    function init_form_fields() {
	      ...
	      'enabled'     => array(
	        ...
	        'label'       => __( 'Enable Epsilon GMO Postpay Payment Payment', 'wc4jp-epsilon' ),
	        'type'        => 'checkbox',
	        'description' => '',
	        'default'     => 'no'
	        ),
	      'title'       => array(
	        ...
	        'default'     => __( 'GMO Postpay Payment (Epsilon)', 'wc4jp-epsilon' )
	        ),
	      'description' => array(
	        ...
	        'default'     => __( 'Pay with your GMO Postpay Payment via Epsilon.', 'wc4jp-epsilon' )
	        ),
			'order_button_text' => array(
				'title'       => __( 'Order Button Text', 'wc4jp-epsilon' ),
				'type'        => 'text',
				'description' => __( 'This controls the description which the user sees during checkout.', 'wc4jp-epsilon' ),
				'default'     => __( 'Proceed to Epsilon GMO Postpay Payment', 'wc4jp-epsilon' )
			),
	      'testmode' => array(
	        'title'       => __( 'Test Mode', 'wc4jp-epsilon' ),
			'type'        => 'checkbox',
			'label'       => __( 'Enable Epsilon Test mode For GMO Postpay Payment', 'wc4jp-epsilon' ),
			'default'     => 'yes',
			'description' => sprintf( __( 'Please check you want to use Epsilon Test mode for GMO Postpay Payment.', 'wc4jp-epsilon' )),
	        ),
		);
		}
	 ...
	 */
	function add_wc_epsilon_pro_gp_gateway( $methods ) {
		$methods[] = 'WC_Gateway_Epsilon_Pro_GP';
		return $methods;
	}

	add_filter( 'woocommerce_payment_gateways', 'add_wc_epsilon_pro_gp_gateway' );
