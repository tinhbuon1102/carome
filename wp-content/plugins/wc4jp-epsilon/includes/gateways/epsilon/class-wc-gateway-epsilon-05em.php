 ...
 *
 * Provides a Epsilon Electric Money Payment Gateway (Link Type).
 *
 * @class 			WC_Epsilon
 ...
 */
class WC_Gateway_Epsilon_Pro_EM extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->id                = 'epsilon_pro_em';
		$this->has_fields        = false;
		$this->method_title      = __( 'Epsilon Electric Money', 'wc4jp-epsilon' );
		...
		$this->st_code = '00000-0000-00010-00000-00000-00000-00000';

        // Create plugin fields and settings
		$this->init_form_fields();
		$this->init_settings();
		$this->method_title       = __( 'Epsilon Electric Money', 'wc4jp-epsilon' );
		$this->method_description = __( 'Allows payments by Epsilon Electric Money in Japan.', 'wc4jp-epsilon' );
	 ...
	 function init_form_fields() {
		...
		$this->form_fields = array(
			'enabled'     => array(
				...
				'label'       => __( 'Enable Epsilon Electric Money Payment', 'wc4jp-epsilon' ),
				'type'        => 'checkbox',
				'description' => '',
				'default'     => 'no'
			),
	      'title'       => array(
	        ...
	        'default'     => __( 'Electric Money (Epsilon)', 'wc4jp-epsilon' )
	        ),
			'description' => array(
	        ...
	        'default'     => __( 'Pay with your Electric Money via Epsilon.', 'wc4jp-epsilon' )
	        ),
			'order_button_text' => array(
				'title'       => __( 'Order Button Text', 'wc4jp-epsilon' ),
				'type'        => 'text',
				'description' => __( 'This controls the description which the user sees during checkout.', 'wc4jp-epsilon' ),
				'default'     => __( 'Proceed to Epsilon Electric Money', 'wc4jp-epsilon' )
			),
	      'testmode' => array(
	        'title'       => __( 'Test Mode', 'wc4jp-epsilon' ),
			'type'        => 'checkbox',
			'label'       => __( 'Enable Epsilon Test mode For Electric Money', 'wc4jp-epsilon' ),
			'default'     => 'yes',
			'description' => sprintf( __( 'Please check you want to use Epsilon Test mode for Electric Money.', 'wc4jp-epsilon' )),
	        ),
		);
		}
	 ...
	 */
	function add_wc_epsilon_pro_em_gateway( $methods ) {
		$methods[] = 'WC_Gateway_Epsilon_Pro_EM';
		return $methods;
	}

	add_filter( 'woocommerce_payment_gateways', 'add_wc_epsilon_pro_em_gateway' );
