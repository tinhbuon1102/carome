	...
	public function get_settings() {
		return apply_filters( 'wc_pre_orders_settings', array(
			...
			array(
				...
				'desc'  => sprintf( __( 'Adjust the message by using %1$s{availability_date}%2$s and %1$s{availability_time}%2$s and %1$s{availability_date_approx}%2$s to represent the product\'s availability date and time.', 'wc-pre-orders' ), '<code>', '</code>' ),
				'type'  => 'title'
			),

			array(
				'title'    => __( 'Single Product Page Message', 'wc-pre-orders' ),
				'desc'     => __( 'Add an optional message to the single product page below the price. Use this to announce when the pre-order will be available by using {availability_date} and {availability_time} and {availability_date_approx}. Limited HTML is allowed. Leave blank to disable.', 'wc-pre-orders' ),
				'desc_tip' => true,
			...
			array(
				...
				'desc'     => __( 'Add an optional message to each pre-order enabled product on the shop loop page above the add to cart button. Use this to announce when the pre-order will be available by using {availability_date} and {availability_time} and {availability_date_approx}. Limited HTML is allowed. Leave blank to disable.', 'wc-pre-orders' ),
				'desc_tip' => true,
			...
			array(
				...
				'desc'  => sprintf( __( 'Adjust the display of the order total by using %1$s{order_total}%2$s to represent the order total and %1$s{availability_date}%2$s and %1$s{availability_date_approx}%2$s to represent the product\'s availability date.', 'wc-pre-orders' ), '<code>', '</code>' ),
				'type'  => 'title'
			),
			...
			array(
				...
				'desc'     => __( 'This controls the order total format when the cart contains a pre-order charged upfront. Use this to indicate how the customer is charged for their pre-order by using {availability_date} {availability_date_approx} and {order_total}.', 'wc-pre-orders' ),
				'desc_tip' => true,
			...
			/*Start of added custom field*/
			array(
				'title'    => __( 'Charged Upfront Order Total Format', 'wc-pre-orders' ),
				'desc'     => __( 'Description in cart page', 'wc-pre-orders' ),
				'desc_tip' => true,
				'id'       => 'wc_pre_orders_upfront_order_cart_messege',
				'default'  => __( 'This payment will be charged upfront', 'wc-pre-orders' ),
				'type'     => 'textarea',
			),
			/*End of added custom field*/
