...
class WC_Pre_Orders_Product {
	...
	public function add_pre_order_product_message() {
		...
		$message = str_replace( '{availability_date_approx}', $this->get_localized_availability_date_approx( $product ), $message );
	...
	public static function get_localized_availability_date( $product, $none_text = '' ) {
		...
		return self::get_localized_availability_date_approx( $product);
	...
	public static function get_localized_availability_date_approx( $product, $string = true ) {
		if ( ! is_object( $product ) ) {
			$product = wc_get_product( $product );
		}
	
		if ( ! $product || ! $date_approx = get_post_meta( $product->is_type( 'variation' ) && version_compare( WC_VERSION, '3.0', '>=' ) ? $product->get_parent_id() : $product->get_id(), '_wc_pre_orders_availability_date_approx', true ) ) {
			return '';
		}
		...
		try {
			if ($string)
			{
				$aDate_approx = explode('__', $date_approx);
				$year = $aDate_approx[0];
				$month = $aDate_approx[1];
				$range = $aDate_approx[2];
				return sprintf( __( '%s年%s月', 'wc-pre-orders' ), $year, $month) . __($range, 'wc-pre-orders'); 
			}
			else 
			{
				$aDate_approx = explode('__', $date_approx);
