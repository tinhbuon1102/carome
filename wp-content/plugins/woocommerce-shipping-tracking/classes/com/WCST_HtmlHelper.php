...
class WCST_HtmlHelper
{
	...
	function shipping_dropdown_options($data, $options, $already_shifted = false, $part = '')
		...
		function render_shipping_companies_tracking_info_configurator_widget($post = null) 
		{
			...
			$data = isset($post) ? $wcst_order_model->get_order_meta($post->ID) /* get_post_custom( $post->ID ) */ : array();
			...
			<div class="wcst_shipping_info_box">
				<ul class="totals">
					...
					<li>
						...
						<span class="wcst_notification_email_description"><?php echo $active_notification_description; ?></span>
					</li>		
			...
			<div id="wcst-additional-shippings">
				<?php if(isset($additiona_companies))
					...
					$dispatch_date = $wcst_time_model->format_data($dispatch_date);
					?>
					<div id="wcst-additiona-shipping-box-<?php echo $index_additiona_companies?>" class="wcst_shipping_info_box">
						<ul class="totals">
							<li>
							...
							<li>
								...
								<input style="margin-bottom:15px;" type="text" class="wcst_dispatch_date" name="_wcst_order_additional_shipping[<?php echo $index_additiona_companies?>][order_dispatch_date]" placeholder="<?php _e('19/02/16 or 15th Dec 2016', 'woocommerce-shipping-tracking'); ?>" value="<?php echo $dispatch_date; ?>"  />
							</li>
							<li>
							...
							<li>
								<input class="" id="wcst_send_shipping_notification_email_<?php echo $index_additiona_companies?>" type="checkbox" value="true" data-id="<?php echo $index_additiona_companies?>" name="wcst_send_shipping_notification_email[<?php echo $index_additiona_companies?>]"><?php _e('Send a notification email', 'woocommerce-shipping-tracking'); ?></input>
								<span class="wcst_notification_email_description"><?php echo $active_notification_description; ?></span>
							</li>	
						</ul>
			...
			<h4 style="margin-top:45px;"><?php _e('Disable email embedding', 'woocommerce-shipping-tracking'); ?> 
				<div class="wcst_tooltip dashicons dashicons-editor-help"><span class="wcst_tooltiptext"><?php _e('This option overrides the <strong>General options -> Email options</strong> settings and will allow you to not embed tracking info in any WooCommerce email', 'woocommerce-shipping-tracking'); ?></span></div>
			</h4>
			...
			function wcst_get_template(index)
			{
				var wcst_add_shipping_company_template = '<div id="wcst-additiona-shipping-box-'+index+'" class="wcst_shipping_info_box">';
					...
					wcst_add_shipping_company_template += '			<input style="margin-bottom:15px;" type="text" name="_wcst_order_additional_shipping['+index+'][trackno]" placeholder="<?php echo str_replace("'","\'",__('Enter Tracking No', 'woocommerce-shipping-tracking')); ?>" value="" class="first" />';
					wcst_add_shipping_company_template += '		</li>';
					wcst_add_shipping_company_template += '		<li>';
					...
					wcst_add_shipping_company_template += '			<span class="wcst_notification_email_description"><?php echo $active_notification_description; ?></span>';
					wcst_add_shipping_company_template += '		</li>';	
