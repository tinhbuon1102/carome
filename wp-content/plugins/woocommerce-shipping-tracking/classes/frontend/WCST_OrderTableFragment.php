...
class WCST_OrderTableFragment
{
	...
	public function display_estimated_date($label, $method )
	{
		...
		if ( ! empty( $current_method_days_from ) && ! empty( $current_method_days_to ) ) 
			$label .= sprintf( __( 'Delivery: %1$s - %2$s days', 'woocommerce-shipping-estimate' ), $current_method_days_from, $current_method_days_to);
		elseif ( empty( $current_method_days_from ) && ! empty( $current_method_days_to ) ) 
			$label .= sprintf( __( 'Delivery: up to %1$s day(s)', 'woocommerce-shipping-estimate' ), $current_method_days_to );
		elseif ( ! empty( $current_method_days_from ) && empty( $current_method_days_to ) ) 
			$label .= sprintf( __( 'Delivery: at least %1$s day(s)', 'woocommerce-shipping-estimate' ), $current_method_days_from);
