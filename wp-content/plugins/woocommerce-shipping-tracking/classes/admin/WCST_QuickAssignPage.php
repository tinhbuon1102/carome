...
class WCST_QuickAssignPage
{
	...
	public function render_page()
	{
		...
		<form action="" method="post" >
			<h1 ><?php _e('Quick assign: how it works?', 'woocommerce-shipping-tracking');?></h1>
			...
			<h2 class="wcst_section_title"><?php _e('1. Select order', 'woocommerce-shipping-tracking');?></h2>
			<p><?php _e('You can search by order id, order date, user id, billing email and billing/shipping first and last name', 'woocommerce-shipping-tracking');?></p>
			<select class="js-data-orders-ajax" id="wcst_select2_order_id" name="order_id" required="required"> </select>
			
			<label style="margin-top:15px"><?php _e('Merge tracking info with the existing one?','woocommerce-shipping-tracking'); ?></label>
			<p><?php _e('If "Yes" option is selected, the existing order shipping trackin info (if any) will be merged with the one inserted now.', 'woocommerce-shipping-tracking');?></p>
			<select name="merge_existing_data">
				<option value="yes"><?php _e('Yes', 'woocommerce-shipping-tracking');?></option>
				<option value="no"><?php _e('No', 'woocommerce-shipping-tracking');?></option>
			</select>
			
			<h2 class="wcst_section_title"><?php _e('2. Assign a shipping', 'woocommerce-shipping-tracking');?></h2>
			<?php $wcst_html_helper->render_shipping_companies_tracking_info_configurator_widget(); ?>
		
			<h2 class="wcst_section_title"><?php _e('3. Change order status?', 'woocommerce-shipping-tracking');?></h2>
