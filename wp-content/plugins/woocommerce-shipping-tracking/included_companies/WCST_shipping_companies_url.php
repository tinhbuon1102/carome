	...
	public static function get_company_url($trackurl,$trackno, $shipping_postcode = '')
	{
		...
		$urltrack = null;
			if ($trackurl == 'VIETNAMPOST'){
				...
				$urltrack = 'https://www.internationalparceltracking.com/Main.aspx#/track/'.$trackno.'/BE/2018';
			}
				...
				//new
				//$urltrack = 'https://www.indiapost.gov.in/VAS/Pages/trackconsignment.aspx';
				$urltrack = 'https://track.trackingmore.com/india-post/en-'.$trackno.'.html';
			}
				...
				$urltrack = 'http://poslaju.com.my/track-trace/#trackingIds='.$trackno;
			}
				...
				//$urltrack = 'http://www.pos.com.my/emstrack/viewdetail.asp?parcelno='.$trackno;
				$urltrack = 'http://www.pos.com.my/postal-services/quick-access/?track-trace#trackingIds='.$trackno;
			}
			...
			if(isset($custom_companies) && is_array($custom_companies))
				foreach ($custom_companies as $index => $company_info) //['name'] and ['url']
					if ($index  == $trackurl ) 
					{
						...
						{
							$code = trim($code);
							//$urltrack =  str_replace("%s", $code, $urltrack);//sprintf($company_info['url'], $trackno) ;
							//$urltrack =  str_replace("%S", $code, $urltrack);//to avoid case sensitive ;
